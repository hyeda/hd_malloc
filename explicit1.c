...
#define PUT_PTR(p, val)  (*(unsigned int**)(void*)(p) = (unsigned int*)(val))
...
#define HDRP(bp)    ((char*)(bp) - 3*WSIZE) // bp보다 1워드 앞에 헤더포인트가 있음.!
#define PREV(bp)    ((char*)(bp) - 2*WSIZE) // 전 가용 포인터
#define NEXT(bp)    ((char*)(bp) - 1*WSIZE) // 후 가용 포인터
...
//     int alloc_size;
//     struct _alloc_block* prev_p;
//     struct _alloc_block* next_p;
// } *alloc_block;
...
int mm_init(void)
{
    ...
    PUT(heap_listp + (1*WSIZE), PACK(2*DSIZE, 1)); // 헤더
    PUT_PTR(heap_listp + (2*WSIZE), NULL); // prev ptr
    PUT_PTR(heap_listp + (3*WSIZE), NULL); // next ptr
    PUT(heap_listp + (4*WSIZE), PACK(2*DSIZE, 1)); // 푸터
    PUT(heap_listp + (5*WSIZE), PACK(0, 1)); // 마지막 워드 위치에 블록 크기 0과 할당 상태 저장. 에필로그 블록. 힙의 끝을 나타내는 역할
...
static void *extend_heap(size_t words)
{
    ...
    bp = (char *)ALIGN((size_t)bp);
    PUT(HDRP(bp), PACK(size, 0)); // 블록의 헤더 위치에 size만큼 가용상태를 기록
    PUT_PTR(NEXT(bp), root); // 먼저 root가 가리키고 있는 애를 새 가용블록 next로 넘긴다.
    root = PREV(bp); // root는 새 가용블록의 주소를 가리킨다.
    PUT_PTR(PREV(bp), NULL); // 이 새 가용블록의 prev는 NULL로 갱신. 
    PUT(FTRP(bp), PACK(size, 0));  // 푸터에도 똑같이!
    PUT(HDRP(NEXT_BLKP(bp- 2*WSIZE) ), PACK(0, 1)); // 다음 블록의 헤더에 에필로그 블록을 기록함.
...
//     size = (words % 2) ? (words+1) * WSIZE : words * WSIZE; // 짝수 워드 단위로 크기 계산
//     if ((long)(bp = mem_sbrk(size)) == -1) // mem_sbrk 호출하여 메모리 확장
//         return NULL;
...
//     // 새 블록을 가용 리스트의 앞에 추가
//     PUT_PTR(NEXT(bp), root); // 새 블록의 next를 기존 root로 설정
//     if (root != NULL) {
//         PUT_PTR(PREV(root), bp); // 기존 root 블록의 prev를 새 블록으로 설정
//     }
//     PUT_PTR(PREV(bp), NULL); // 새 블록의 prev는 NULL로 설정
//     root = bp; // root를 새 블록으로 업데이트

//     PUT(HDRP(NEXT_BLKP(bp)), PACK(0, 1)); // 다음 블록의 헤더에 에필로그 블록을 기록함.

//     return coalesce(bp); 
// }
// void *add_alloc_list(bp)
// {
...
static void *find_fit(size_t alloc_size) // malloc에서 이미 요청한 크기에 헤더와 푸터를 포함한 크기를 alloc에 넣음.
{
    ...
    for (bp = root + 2*WSIZE; GET_SIZE(HDRP(bp)) > 0; bp = NEXT(bp) + 2*WSIZE) { // get_size == 0(에필로그 블록)이 될때 까지 반복해서 탐색하라는 의미. 
...
static void place(void *bp, size_t alloc_size) 
{
    ...
    if ((block_size - alloc_size) >= (2*DSIZE)) { // 블럭에서 할당된 크기를 뺐는데도 최소 한 블럭 만들수 있는 크기 나오면 분할
        ...
        PUT_PTR(NEXT(bp+ 2*WSIZE) , root); // 먼저 root가 가리키고 있는 애를 새 가용블록 next로 넘긴다.
        root = PREV(bp); // root는 새 가용블록의 주소를 가리킨다.
        PUT_PTR(PREV(bp), NULL); // 이 새 가용블록의 prev는 NULL로 갱신. 
...
{   
    bp -= 2*WSIZE;
    size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(bp) + 2*WSIZE)); // 앞블럭의 가용상태
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(bp) + 2*WSIZE)); // 뒷블럭의 가용상태
    size_t size = GET_SIZE(HDRP(bp + 2*WSIZE)); // 현재 블럭의 크기
    ...
    if (prev_alloc && next_alloc ) { // 둘다 할당 된 상태
        ...
        size += GET_SIZE(HDRP(NEXT_BLKP(bp) + 2*WSIZE)); // 사이즈를 뒷블럭 크기만큼 키움.
        bp += 2*WSIZE;
        ...
        size += GET_SIZE(HDRP(PREV_BLKP(bp) + 2*WSIZE)) + GET_SIZE(FTRP(NEXT_BLKP(bp) + 2*WSIZE)); // 앞 뒤 블록 둘다 가용상태
        bp = PREV_BLKP(bp) + 2*WSIZE; // 헤더를 앞블럭의 헤더위치로 옮겨 줘야 함.
